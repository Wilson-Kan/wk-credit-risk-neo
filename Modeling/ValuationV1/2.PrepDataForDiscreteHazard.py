...
future_join_df = full_train_df.select(
    "accountId", F.col("referenceDate").alias("future_date"), F.col("avg_utilization").alias("target_util")
)

model_df = full_train_df_t_trans.join(future_join_df, ["accountId", "future_date"], "inner")
...
model_df_pd = model_df[["brand", "referenceDate", "target_util"]].groupby(["brand", "referenceDate"]).mean("target_util").toPandas()
...
non_hbc_df = model_df.filter(model_df.brand != "HBC")

model_non_hbc_temp_df = non_hbc_df.toPandas()

# COMMAND ----------

model_non_hbc_df = model_non_hbc_temp_df.drop(columns=['accountId', 'productName', 'productTypeName', 'cardType', 'cvs_date', 'source', 'future_date', 'rn'])

# COMMAND ----------

model_non_hbc_df['mth_tot_feeRevenue'] = model_non_hbc_df['mth_tot_feeRevenue'].astype(float)
model_non_hbc_df['mth_tot_rewardsCost'] = model_non_hbc_df['mth_tot_rewardsCost'].astype(float)
model_non_hbc_df['mth_tot_rewardDollarsEarned'] = model_non_hbc_df['mth_tot_rewardDollarsEarned'].astype(float)
model_non_hbc_df['avg_bal'] = model_non_hbc_df['avg_bal'].astype(float)
model_non_hbc_df['max_bal'] = model_non_hbc_df['max_bal'].astype(float)
model_non_hbc_df['avg_utilization'] = model_non_hbc_df['avg_utilization'].astype(float)
model_non_hbc_df['max_utilization'] = model_non_hbc_df['max_utilization'].astype(float)
model_non_hbc_df['mthend_utilization'] = model_non_hbc_df['mthend_utilization'].astype(float)
model_non_hbc_df['target_util'] = model_non_hbc_df['target_util'].astype(float)
model_non_hbc_df['mth_tot_cashAdvanceVolume'] = model_non_hbc_df['mth_tot_cashAdvanceVolume'].astype(float)
model_non_hbc_df['mth_tot_rewardCashOutVolume'] = model_non_hbc_df['mth_tot_rewardCashOutVolume'].astype(float)
model_non_hbc_df['mth_tot_grossInterestRevenue'] = model_non_hbc_df['mth_tot_grossInterestRevenue'].astype(float)
model_non_hbc_df['mth_tot_grossPurchaseVolume'] = model_non_hbc_df['mth_tot_grossPurchaseVolume'].astype(float)
model_non_hbc_df['mth_tot_grossRefundVolume'] = model_non_hbc_df['mth_tot_grossRefundVolume'].astype(float)
model_non_hbc_df['mth_tot_netInternationalPurchaseVolume'] = model_non_hbc_df['mth_tot_netInternationalPurchaseVolume'].astype(float)
model_non_hbc_df['mth_tot_interchangeRevenue'] = model_non_hbc_df['mth_tot_interchangeRevenue'].astype(float)
model_non_hbc_df['mthend_bal'] = model_non_hbc_df['mthend_bal'].astype(float)
model_non_hbc_df['incrementalCreditLimitOfferAmount'] = model_non_hbc_df['incrementalCreditLimitOfferAmount'].astype(float)
model_non_hbc_df['creditLimit'] = model_non_hbc_df['creditLimit'].astype(float)
model_non_hbc_df['latestStatementNewBalanceAmountDollars'] = model_non_hbc_df['latestStatementNewBalanceAmountDollars'].astype(float)
model_non_hbc_df['latestStatementPastDueAmountDollars'] = model_non_hbc_df['latestStatementPastDueAmountDollars'].astype(float)
model_non_hbc_df['changeCreditLimit'] = model_non_hbc_df['changeCreditLimit'].astype(float)
model_non_hbc_df['availableCreditLimit'] = model_non_hbc_df['availableCreditLimit'].astype(float)
model_non_hbc_df['latestStatementMinimumPaymentAmountDollars'] = model_non_hbc_df['latestStatementMinimumPaymentAmountDollars'].astype(float)
model_non_hbc_df['mth_tot_netPurchaseVolume'] = model_non_hbc_df['mth_tot_netPurchaseVolume'].astype(float)
model_non_hbc_df['mth_tot_netPurchaseVolume'] = model_non_hbc_df['mth_tot_netPurchaseVolume'].astype(float)

model_non_hbc_df['latestStatementPaymentDueDate_mth'] = model_non_hbc_df['latestStatementPaymentDueDate'].astype("datetime64[ns]").dt.month
model_non_hbc_df['referenceDate_mth'] = model_non_hbc_df['referenceDate'].astype("datetime64[ns]").dt.month
model_non_hbc_df['openedAt_mt_mth'] = model_non_hbc_df['openedAt_mt'].astype("datetime64[ns]").dt.month

# COMMAND ----------

model_non_hbc_df_x = model_non_hbc_df.drop(columns=['referenceDate','channel','openedAt_mt','latestStatementPaymentDueDate'])
...
X = model_non_hbc_df_x.drop(columns=['target_util'])
y = model_non_hbc_df_x['target_util']
...
perm_importance_df = pd.DataFrame({'feature': X_test.columns, 'importance_mean': perm_importance.importances_mean, 'importance_std': perm_importance.importances_std})
