...
default_list = spark.sql(
  f"""
    select distinct applicationId, 1 as def
      FROM
...
score_me = raw_data.alias("a").join(default_list.alias("b"), raw_data.applicationId == default_list.applicationId, how="left").select("a.*", "b.def")
...
for i in range(len(model_list)):
  ...
  test_dat.loc[:,'raw_pred'] = - 1 * bst.predict_proba(test_dat.loc[:,bst.feature_names_in_])[:,1]
  auc = 1
...
for i in features:
  X_train['temp'], X_b = pd.cut(X_train[i], bins=10, include_lowest=True, retbins=True)
  psi_group = X_train[['temp']].groupby('temp', observed=True).size() / X_train.shape[0]
